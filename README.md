...
#### 3. 校验补全（可选）
- 第二次 LLM 调用，专门审查首次分析遗漏
- 重点识别：第一人称叙述者("我")、男性角色、仅以角色名提及的人物
- 合并两次结果，生成完整分析

#### 4. 特殊处理
- **第一人称叙述者**：自动识别"我"、"主角"等，推断性别和角色名
...
- **校验补全机制**（可选）：二次调用 LLM 审查遗漏内容，智能合并结果。
